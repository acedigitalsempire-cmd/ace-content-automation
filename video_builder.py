...
def generate_voiceover(text, index):
    ...
    if size < 1000:
        print(f"Audio too small - error response saved")
        return None
...
def add_branded_overlay(bg_image_path, photo_path, scene_text, output_path):
    ...
    for line in lines:
        ...
        w = bbox[2] - bbox[0]
        x = (VIDEO_WIDTH - w) // 2
        draw.text((x, text_y), line, font=font_large, fill=tuple(GOLD))
        text_y += 65

    # Brand name top left
    draw.text((20, 20), "Ace Digitals Global", font=font_brand, fill=tuple(GOLD))

    # Handle top right
    handle_text = "@DigitalUche"
    bbox = draw.textbbox((0, 0), handle_text, font=font_brand)
    handle_w = bbox[2] - bbox[0]
    draw.text((VIDEO_WIDTH - handle_w - 20, 20),
              handle_text, font=font_brand, fill=tuple(WHITE))
...
def build_video(scenes, image_paths, photo_path):
    ...
    for i, (scene, image_path) in enumerate(zip(scenes, image_paths)):
        ...
        try:
            ...
            print(f"Audio fallback for scene {i+1}: {e}")
            audio_path = create_silent_audio(default_duration, i)
            audio_clip = AudioFileClip(audio_path)
