...
async def script_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Generating your script...")
...
async def upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"I'll number them in the order I receive them\\.\n\n"
...
async def receive_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not video:
        await update.message.reply_text("Please send a video file\\.", parse_mode="MarkdownV2")
        return
    ...
    await update.message.reply_text(f"⬇️ Downloading clip {current_count}...")
    ...
    try:
        ...
        if len(videos) >= total_scenes:
            ...
            await update.message.reply_text(
                f"✅ Clip {current_count} received\\! \\({file_size:,} bytes\\)\n"
                f"📊 {len(videos)}/{total_scenes} clips uploaded\\.\n"
...
async def done_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        results = []
        results.append("📊 *Publishing Results*\n")
