...
# ---------------------------------------------------------------------------
# Health server — satisfies Render's port scan and UptimeRobot /healthz ping
# ---------------------------------------------------------------------------
...
@_flask_app.route("/")
def _health():
    return "ACE Content Bot is running.", 200
...
async def script_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    logger.info("SCRIPT command from chat_id: %s", chat_id)
...
async def upload_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    logger.info("UPLOAD command from chat_id: %s", chat_id)

    if "current_script" not in context.user_data:
        await update.message.reply_text(
            "⚠️ No script found. Run /script first."
        )
    ...
    total_scenes = len(context.user_data["current_script"]["scenes"])
...
async def receive_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    logger.info("VIDEO received from chat_id: %s", chat_id)

    if not context.user_data.get("upload_mode"):
        await update.message.reply_text(
            "⚠️ Send /upload first to start uploading clips."
        )
        return
    ...
    try:
        ...
        logger.error("Error downloading clip: %s", e)
        await update.message.reply_text(
            f"❌ Error downloading clip {current_count}.\n\nError: {e}\n\nTry sending it again."
        )
...
async def done_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    logger.info("DONE command from chat_id: %s", chat_id)
    ...
    if not videos:
        await update.message.reply_text(
            "⚠️ No clips uploaded yet. Send /upload first."
        )
        return ConversationHandler.END
    ...
    if len(videos) < total_scenes:
        await update.message.reply_text(
            f"⚠️ Only {len(videos)}/{total_scenes} clips uploaded.\n"
            f"Send the remaining clips or /cancel to start over."
        )
        return WAITING_FOR_VIDEOS
    ...
    try:
        ...
        lines.append(f"Instagram: {'✅ Published' if ig_result else '❌ Failed'}")
        lines.append(f"Facebook:  {'✅ Published' if fb_result else '❌ Failed'}")

        if yt_result:
            yt_id = yt_result.get("id", "")
            lines.append(f"YouTube:   ✅ https://youtube.com/shorts/{yt_id}")
        else:
            lines.append("YouTube:   ❌ Failed")
        ...
        traceback.print_exc()
        logger.error("Processing/publishing failed: %s", e)
        await update.message.reply_text(
            f"❌ Processing failed.\n\nError: {e}\n\nTry /upload again or /cancel to reset."
        )
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    logger.info("CANCEL command from chat_id: %s", update.effective_chat.id)
    context.user_data["uploaded_videos"] = []
    context.user_data["upload_mode"] = False
    await update.message.reply_text(
...
def main():
    ...
    logger.info("ACE Content Studio Bot is running...")
    app.run_polling(
